...
use ndarray::{s, ArrayView, IxDyn, ViewRepr};
...
impl YoloTir {
    pub fn new(name: &str, context: &rclrs::Context) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let nms_threshold = node
            .declare_parameter("nms_threshold")
            .default(0.50)
            .mandatory()
            .unwrap()
            .get() as f32;
        ...
        let conf_idx = node
            .declare_parameter("conf_idx")
            .default(4)
            .mandatory()
            .unwrap()
            .get() as usize;

        let cls_idx_offset = node
            .declare_parameter("cls_idx_offset")
            .default(5)
            .mandatory()
            .unwrap()
            .get() as usize;
        ...
        assert!(
            cls_idx_offset > 0,
            "Confidence idx offset cannot be negative"
        );
        assert!(
            classes.len() > 0,
        ...
        assert_eq!(
            &model_path[(model_path.len() - 5)..model_path.len()],
            ".onnx"
        );
    ...
    pub fn publish(
        &self,
        output_vec: Vec<OrtOwnedTensor<f32, IxDyn>>,
        ratio_w: f32,
        ratio_h: f32,
        infer_info: Arc<InferenceInfo>,
        ...
        for n in 0..infer_info.n_proposal {
            ...
            if obj_score > self.conf_score {
                ...
                // softmax
                let exp_array = output_vec[0]
                    .slice(s![0, n, self.cls_idx_offset..])
                    .mapv(f32::exp);
                // println!("{}", exp_array);
    ...
    pub fn process_image(
        img_msg: ImageMsg,
        infer_info: Arc<InferenceInfo>,
    ) -> Vec<ndarray::Array<f32, IxDyn>> {
        // easiest way to create an array
        // size is [batch, channel, width, height]
        let mut img_data =
            ndarray::Array::<f32, IxDyn>::zeros(IxDyn(infer_info.input0_shape.as_ref()));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::thread::spawn(move || -> Result<(), Box<dyn std::error::Error + Send>> {
        ...
        let mut session = environment
            ...
            .unwrap()
            .with_model_from_file(&clone_infer.model_path[..])
            .unwrap();
        ...
        loop {
            ...
            if msg.is_none() {
                ...
                let input_tensor_values =
                    YoloTir::process_image(img_msg.clone(), infer_info.clone());
